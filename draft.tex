...
\begin{figure}[h!]
  ...
  \begin{subfigure}[b]{0.49\textwidth}
    ...
    \caption{SBM-WS}
  \end{subfigure}
  \caption{Evolution of the fraction of nodes with opinion 0 ($P_0(t)$) over time for an initial $50/50$ distribution. Four different network models are used (each with an average degree around 10 and a total number of nodes equal to 1000): \textbf{(a)} Erd\H{o}s-R\'{e}nyi model (ER), $p=0.01$; \textbf{(b)} Watts-Strogatz model (WS), $K = 10$, two slightly different values of the clustering coefficient are used ($\left<C\right> \sim 0.55$ and $\left<C\right> \sim 0.60$); \textbf{(c)} Stochastic block model (SBM) with 10 communities of size 100 ($10\times100$), a high modularity case ($p_{cl} = 0.1,\ p_{add} = 0.001,\ Q \sim 0.9$) and a low modularity case ($p_{cl} = 0.03,\ p_{add} = 0.008,\ Q \sim 0.2$) are depicted; \textbf{(d)} Stochastic Watts-Strogatz block model (SBM-WS) with 10 communities of size 100 ($10\times100$), each community has the following WS parameters: $K = 10,\ \beta = 0.01$, $\left<C\right> \sim 0.55$ and $Q \sim 0.9$. For each model two different filtering algorithms are compared to each other: the REC and the PR filtering algorithm. Each figure is the result of $10^2$ independent simulations and data points are shown for every 50 time steps to improve visualization.}%more explanation?
\end{figure}
\newpage
