...
Lots of studies have been performed on this model. In this thesis, however, we will not make use of the voter model. Hence, we will not go into further detail. Interested readers are referred to references \cite{Castellano2009}, \cite{Fernandez2014} and related references therein.% cite papers that have done studies on this model??
...
In this chapter the results are shared and discussed. The chapter in subdivided into three main sections
\begin{itemize}
...
In this section we will discuss some results related to the network structure and network topologies of the different network models that we used. We will mainly focus on the effect of a high clustering coefficient and a community structure. Two different opinion distributions are investigated
\begin{itemize}
...
\newline
Four different network models are implemented, each with a total number of nodes equal to $N = 10^3$ and an average degree around 10
\begin{itemize}
...
\begin{figure}[H]
  \begin{subfigure}[b]{0.49\textwidth}
  	\includegraphics[width=8cm, height=7cm]{fraction_of_opinions_ER_001_50-50_PR_REC.png}
    \caption{ER}
  \end{subfigure}
  \begin{subfigure}[b]{0.49\textwidth}
  	\includegraphics[width=8cm, height=7cm]{fraction_of_opinions_WS_50-50_PR_REC.png}
    \caption{WS}
  \end{subfigure}
  \begin{subfigure}[b]{0.49\textwidth}
  ...
  \caption[Evolution of the fraction of nodes with opinion 0 ($P_0(t)$) over time for an initial $50/50$ opinion distribution]{Evolution of the fraction of nodes with opinion 0 ($P_0(t)$) over time for an initial $50/50$ distribution. Four different network models are used (each with an average degree around 10 and a total number of nodes $N = 10^3$): \textbf{(a)} Erd\H{o}s-R\'{e}nyi model (ER), $p=0.01$; \textbf{(b)} Watts-Strogatz model (WS), $K = 10$, two slightly different values of the clustering coefficient are used ($\left<C\right> \sim 0.55$ and $\left<C\right> \sim 0.60$); \textbf{(c)} Stochastic block model (SBM) with 10 communities of size 100, a high modularity case ($p_{\text{cl}} = 0.1,\ p_{\text{add}} = 0.001,\ Q \sim 0.9$) and a low modularity case ($p_{\text{cl}} = 0.03,\ p_{\text{add}} = 0.008,\ Q \sim 0.2$) are depicted; \textbf{(d)} Stochastic Watts-Strogatz block model (SBM-WS) with 10 communities of size 100, each community has the following WS parameters: $K = 10,\ \beta = 0.01$ and $p_{\text{add}} = 0.001$, $\left<C\right> \sim 0.55$ and $Q \sim 0.9$. For each model two different filtering algorithms are compared to each other: the REC and the PR filtering algorithm. Each plot is the average of $10^2$ independent simulations and data points are shown for every 50 time steps to improve visualization.}%more explanation?
\label{ev_op_50_50}
\end{figure}
\newpage
\noindent
...
\newline
Once again, we look at four different network models each with a total number of nodes equal to $N = 10^3$ and an average degree around $10$
\begin{itemize}
	\item The Erd\H{o}s-R\'{e}nyi model (ER) with edge probability $p = 0.01$.
...
\newline
From Figure \ref{ev_op_20_80}, it is clear that the minority opinion 0 eventually ceases to exist for all the network models, $P_0(500) \sim 0$. This result differs from the conclusion that prof. N. Perra and prof. L.E.C. Rocha made in the paper \textit{"Modelling opinion dynamics in the age of algorithmic personalization"} (see reference \cite{Perra2019}). They found that the minority opinion was able to survive over time, even for the PR method, and reported a minority opinion fraction at $t=500$ between $P_0(500) = 0.06$ and $P_0(500) = 0.09$ \cite{Perra2019}. % explain further, but first test WS, K=6, beta=0.01, N=1000, 20-80, PR 

% Talk about effect network topologies?? See paper + simulation with N = 10.000 and N = 100.000 --> mention possible discrepancies and effect of network size??. Having a high clustering coefficient and/or a clear community structure plays no role in the e
\newpage
\begin{figure}[H]
  \begin{subfigure}[b]{0.49\textwidth}
  ...
  \end{subfigure}
  \captionsetup{format=plain}
  \caption[Evolution of the fraction of nodes with opinion 0 ($P_0(t)$) over time for an initial $20/80$ opinion distribution]{Evolution of the fraction of nodes with opinion 0 ($P_0(t)$) over time for an initial $20/80$ distribution. Four different network models are used (each with an average degree around 10 and a total number of nodes $N = 10^3$): \textbf{(a)} Erd\H{o}s-R\'{e}nyi model (ER), $p=0.01$; \textbf{(b)} Watts-Strogatz model (WS) with $K = 10$, $\beta = 0.06$ and $\left<C\right> \sim 0.55$; \textbf{(c)} Stochastic block model (SBM) with 10 communities of size 100, a high modularity case ($p_{\text{cl}} = 0.1,\ p_{\text{add}} = 0.001,\ Q \sim 0.9$) and a low modularity case ($p_{\text{cl}} = 0.03,\ p_{\text{add}} = 0.008,\ Q \sim 0.2$) are depicted; \textbf{(d)} Stochastic Watts-Strogatz block model (SBM-WS) with 10 communities of size 100, each community has the following WS parameters: $K = 10,\ \beta = 0.01$ and $p_{\text{add}} = 0.001$, $\left<C\right> \sim 0.55$ and $Q \sim 0.9$. For each model two different filtering algorithms are compared to each other: the REC and the PR filtering algorithm. Each plot is the average of $10^2$ independent simulations and data points are shown for every 50 time steps to improve visualization.}
\label{ev_op_20_80}
\end{figure}
\newpage
\noindent
...
\newpage
\noindent
In increasing order of modularity, the used network models are (all with a total number of nodes $N = 10^3$ and an average degree around 10)
\begin{itemize}
...
We will again implement four different network models in order to investigate the possible interplay between network structure and stubbornness, these models are (each with a total number of nodes $N = 10^3$ and an average degree around 10)
\begin{itemize}
	\item The Erd\H{o}s-R\'{e}nyi model (ER) with edge probability $p = 0.01$.
...
\newline
We start by making a fraction of the nodes (fracRes) completely stubborn ($r_i = 1$). We go from no stubborn nodes, fracRes $=0$, to all the nodes are stubborn, fracRes $=1$.\\
\newline
