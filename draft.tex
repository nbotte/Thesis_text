...
\end{align}
Watts and Strogatz then showed by numerical simulation that the average distance for the networks with $0 < p < 1$ is comparable with the average distance of a random graph (which is small), even for small $p$ \cite{Newman2000}. The clustering coefficient is approximately given by   \cite{Barrat1999}
\begin{equation}
...
\begin{itemize}
	...
	\item If $r_i = 1$, the node is completely stubborn and there is no change of convincing him/here of the other idea
...
\newline
In the threshold model one introduces a notion of stubbornness by introducing a threshold parameter $T$. This parameter can be either the same for all the nodes ($T$) or can be made individual ($T_i$). The main idea is then that nodes can only change their opinion to the other opinion if the fraction of friends with that opinion ($p$) is bigger than $T$ ($p > T$). The idea behind this threshold model is that people are a bit hesitant to change their opinion, but if they have enough friends with another opinion they might be convinced of that opinion as well. The more persistent you are, the more friends you need in order to be convinced of another idea. \\
There are different possibilities to implement this kind of model. One way, which makes use of the majority model, is as follows. First the fraction of friends with opinion A and opinion B, $p_A$ and $p_B$ respectively, is determined; then the following rules are used to update the opinion
\begin{itemize}
...
% Talk about probabilistic implementation?
\newline
...
The probability that a node is connected to $k$ other nodes and not to $N-1-k$ others is given by
\begin{equation}
	p^k (1-p)^{N-1-k}
\end{equation}
