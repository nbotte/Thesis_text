...
\begin{itemize}
	...
	\item A partition of the node set $\{1, 2, ..., N\}$ into disjoint subsets $C_1, ..., C_k$, these are the communities
	\item A symmetric $k\times k$ matrix $M$; the diagonal elements $M_{ii}$, $i = 1, ..., k$ represent the edge probabilities inside the community $C_i$ and the other elements $M_{ij}$, $i, j = 1, ..., k$ and $i \neq j$, give the edge probabilities between nodes in community $C_i$ and nodes in community $C_j$
\end{itemize}
% discuss case where probailities on diag are bigger than off-diag --> assortative SBM
% NOT FINISHED
% clauset2017 http://tuvalu.santafe.edu/~aaronc/courses/5352/csci5352_2017_L6.pdf
