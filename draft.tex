...
\end{equation}
The total degree of node $i$ is then $k_i = k_i^{\text{in}} + k_i^{\text{out}}$. For weighted graphs the degree is easily generalized to the weighted degree $s_i$, also often called strength \cite{Ioannis2007}
\begin{equation}
	s_i = \sum_{j=1}^N w_{ij}.
\end{equation}
%start talking about degree distribution (global, degree = local)
  

% talk about what is a graph, talk about degree distribution, clustereing, average path length, etc (in different subsections)
