...
% maybe put calculations in appendix?
...
Most real networks are however sparse, which means that $\left<k\right> \ll N$. In this case the degree distribution is well approximated by a Poisson distribution \cite{Albert2014} 
\begin{equation}
	p_k = \mathrm{e}^{-\left<k\right>} \frac{\left<k\right>^k}{k!}
\end{equation}
% In appendix??
The derivation goes as follows. We begin with the exact binomial distribution (Eq.(\ref{degDistRan})) which characterizes a random graph. In the case $k \ll N$, we can simplify the binomial in the following way \cite{Albert2014}
...
\begin{equation}
	\ln(1+x) = \sum_{n=1}^{\inf}\frac{(-1)^{n+1}}{n}x^n = x - \frac{x^2}{2} + \frac{x^3}{3} - ...  \qquad \forall \ |x| \leq 1
\end{equation}
